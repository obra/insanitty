...
| insanitty's **Swift** client wired into the GUI | **REAL (subprocess), one-shot** (`makeRemoteWorkspacePage` → `injectRemoteGrid`) | The "remote (QUIC)" workspace paints a surface from insanitty's OWN remote stack: the app launch-or-resumes the helper, then its **SPKI-pinned native Swift QUIC client** (`tools/quic-client`) attaches over QUIC and renders the grid (`RemoteGridRenderer` — styled cells, 2 tests) — no bash/Python/Go-probe. Verified rendering the real remote grid (`scripts/e2e-remote-gui.sh`, `docs/images/e2e-7-remote-in-gui.png`). One-shot paint (re-injected ≤3×), **read-only** (no input), no datagram deltas; the QUIC client runs as a subprocess (in-process msquic is the remaining step). |
| Native Swift QUIC client | **REAL but minimal & not in the app** (`tools/quic-client`) | Binds msquic, parses the helper's real `FANTASTTY_REMOTE` bootstrap line (`--bootstrap`, via the tested `RemoteBootstrapLine`), **enforces SPKI cert pinning** — validates `hex(SHA256(SubjectPublicKeyInfo))` against the bootstrap pin in the peer-certificate callback (correct pin connects, a tampered pin is rejected: `scripts/e2e-native-quic-pinning.sh`) — attaches, and decodes a `paneKeyframe` via `RemoteGridProtocol` (`scripts/e2e-native-quic.sh`). Remaining: datagram deltas, input/predictive echo, and wiring it into `app/` (the GUI remote workspace is still the bash/Python demo). |
