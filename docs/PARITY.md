...
| Fantastty | insanitty | Notes |
|---|---|---|
| **tmux control mode (`-CC`)**: `TmuxControlClient`, protocol parser, V2 runtime, layout parser/mapper — windows↔tabs, panes↔splits, live layout sync | **PARTIAL — interactive single pane** (`TmuxControlParser`/`TmuxLayoutParser` + live `TmuxControlClient`, `app/TmuxControl.swift`) | Parser foundation (12 tests) **plus a live, interactive control client**: spawns `tmux -CC attach` in a PTY (`ins_pty_spawn`/`forkpty`), reads the protocol on the GTK main loop (`g_unix_fd_add`), renders a pane by injecting `%output` into a silent Ghostty surface, **and routes keystrokes back via `send-keys`** — typed input runs in the real session and the result paints back (`scripts/e2e-tmux-control.sh`). **Panes→splits**: a window's `%layout-change`/`#{window_layout}` is parsed and mapped onto a `GtkPaned` tree of silent surfaces (one per pane), reused across relayouts; the client reports its size via `refresh-client -C` so panes render at the right size (`scripts/e2e-tmux-control-split.sh` builds **and renders** a 2-pane split). Remaining: fan multiple windows→tabs. The default workspaces still use plain `tmux new-session -A`. |
