...
| insanitty's **Swift** client wired into the GUI | **REAL — in-process, multi-pane** (`RemoteQuicFetcher`, `app/RemoteQuic.swift`) | The "remote (QUIC)" workspace fetches the workspace **in-process over QUIC** (msquic linked into the app, **SPKI-pinned**), collects the `workspaceSnapshot` + a `paneKeyframe` per pane, maps the window layout onto a **GtkPaned split** (one inert surface per pane, reusing the control-mode `buildPaneTree`), and renders each pane with `RemoteGridRenderer` (styled cells via ANSI SGR). Single-pane fetch verified **live** (`scripts/e2e-remote-gui.sh`, `docs/images/e2e-7-remote-in-gui.png`); multi-pane render verified via fixture (`scripts/e2e-remote-multipane.sh`, `docs/images/remote-multipane.png`) — a *live* multi-pane session needs the helper's `--tmux-session`, whose daemon doesn't become ready in this env. **Interactive input**: keystrokes typed in a remote pane are captured, encoded, and forwarded over the live connection as a `sendKeys` request (+ a `requestKeyframe` so the echo repaints) — verified to the QUIC boundary (`scripts/e2e-remote-input.sh` asserts the bytes are forwarded over the live connection). The end-to-end echo isn't asserted: the only available remote workspace is the helper's default direct-shell source, whose `SendKeys` is a no-op stub (`return nil`) — input executes only on tmux-backed workspaces (same `--tmux-session` blocker). Remaining: datagram deltas, predictive echo, continuous streaming. |
